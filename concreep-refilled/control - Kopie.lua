...
DEFAULT_RADIUS_OF_GHOST_CLEARANCE_CHECK_HYPERCREEP = 5
...
function hypercreep_builder(target_x_pos,target_y_pos,offset,roboport)
	...
	local build_vert_half_pos_y = target_y_pos-half_distance_y+offset
	local build_vert_pos_y = target_y_pos+offset
	local build_horz_half_pos_x = target_x_pos-half_distance_x
	...
	if (
		...
		surface.can_place_entity{name="entity-ghost", position={target_x_pos,build_vert_pos_y}, inner_name="big-electric-pole", force=force} and
		has_value(global.placed_roboports_positions,{target_x_pos,target_y_pos}) == false and
		has_value(global.placed_poles_positions,{target_x_pos,build_vert_pos_y}) == false -- doing this as the game does place multiple power poles on top of each other (and roboports), if we do it all in one frame - not entirely sure how to circumvent that any other way than this
	) then
		...
		table.insert(global.placed_roboports_positions,{target_x_pos,target_y_pos})
		surface.create_entity{name="entity-ghost", position={target_x_pos,build_vert_pos_y}, inner_name="big-electric-pole", force=force, expires=false}
		table.insert(global.placed_poles_positions,{target_x_pos,build_vert_pos_y})
	end
		if (
			target_y_pos ~= roboport_y_pos and
			surface.can_place_entity{name="entity-ghost", position={target_x_pos,build_vert_half_pos_y}, inner_name="big-electric-pole", force=force} and
			has_value(global.placed_poles_positions,{target_x_pos,build_vert_half_pos_y}) == false
		) then
			surface.create_entity{name="entity-ghost", position={target_x_pos,build_vert_half_pos_y}, inner_name="big-electric-pole", force=force, expires=false}
			table.insert(global.placed_poles_positions,{target_x_pos,build_vert_half_pos_y})
		end
		if (
			target_x_pos ~= roboport_x_pos and
			surface.can_place_entity{name="entity-ghost", position={build_horz_half_pos_x,build_vert_pos_y}, inner_name="big-electric-pole", force=force} and
			has_value(global.placed_poles_positions,{build_horz_half_pos_x,build_vert_pos_y}) == false
		) then
			surface.create_entity{name="entity-ghost", position={build_horz_half_pos_x,build_vert_pos_y}, inner_name="big-electric-pole", force=force, expires=false}
			table.insert(global.placed_poles_positions,{build_horz_half_pos_x,build_vert_pos_y})
		end
	game.print(#global.placed_poles_positions)
end
...
function hypercreep(roboport)
	...
	hypercreep_builder(roboport_x + logistic_diameter, roboport_y                    , offset_power_poles, roboport)
	hypercreep_builder(roboport_x - logistic_diameter, roboport_y                    , offset_power_poles, roboport)
	hypercreep_builder(roboport_x                    , roboport_y + logistic_diameter, offset_power_poles, roboport)
	hypercreep_builder(roboport_x                    , roboport_y - logistic_diameter, offset_power_poles, roboport)
	hypercreep_builder(roboport_x + logistic_diameter, roboport_y + logistic_diameter, offset_power_poles, roboport)
	hypercreep_builder(roboport_x - logistic_diameter, roboport_y - logistic_diameter, offset_power_poles, roboport)
	hypercreep_builder(roboport_x + logistic_diameter, roboport_y - logistic_diameter, offset_power_poles, roboport)
	hypercreep_builder(roboport_x - logistic_diameter, roboport_y + logistic_diameter, offset_power_poles, roboport)
end
